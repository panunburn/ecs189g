#!/usr/bin/python
#This is the original trigram model
# David Bamman
# 2/14/14
#
...
for (prevprevtag,prevtag,tag) in tri_transitions:
	print "trans %s %s %s %s" % (prevprevtag, prevtag, tag, float(tri_transitions[(prevprevtag, prevtag, tag)]) / bi_transitions[(prevtag,tag)])
